...
export const ALUNOS = Array.from({ length: 247 }, (_, i) => {
  ...
  return {
    ...
    nascimento: `${String(srndN(1,28)).padStart(2,"0")}/${String(srndN(1,12)).padStart(2,"0")}/${srndN(1975,2005)}`,
    naturalidade: srnd(CIDADES_NAT),
    genero_id:    srnd(GENERO_ID),
    raca:         srnd(RACAS),
    endereco: `Rua ${srnd(RUAS)}, ${srndN(10,999)} — ${srnd(BAIRROS)}, Fortaleza/CE`,
    familia_membros: srndN(1, 8),
    filhos:          srndN(0, 4),
    emprego:         srnd(EMPREGOS),
    beneficio: bs ? srnd(BENEFICIOS) : "Nenhum",
    problema_saude: srnd(PROBLEMAS),
    restricao:      srnd(RESTRICOES),
    pcd,
    bs,
    deficiencia: pcd ? srnd(DEFIC) : "—",
  };
});
